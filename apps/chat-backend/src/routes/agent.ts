  ...
  app.post("/v1/threads/:threadId/interrupts/:interruptId/resume", async (request, reply) => {
    ...
    try {
      ...
      return await controller.resumeRecovery({
        threadId,
        checkpointReader: async () => ({ ...checkpoint, messages: checkpoint.messages ?? [], evidence: checkpoint.evidence ?? [], session_id: checkpoint.session?.session_id, session_revision: checkpoint.session?.revision, session_storage_ref: checkpoint.session?.storage_ref }),
