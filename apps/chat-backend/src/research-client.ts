...
function parseInterruptCheckpoint(value: unknown): InterruptCheckpoint { if(!isObject(value)||typeof value.checkpoint_id!=="string"||typeof value.interrupt_id!=="string"||typeof value.run_id!=="string"||typeof value.operation_id!=="string"||typeof value.nonce!=="string"||typeof value.last_event_seq!=="number"||!isObject(value.last_event)||typeof value.last_event.type!=="string"||!isObject(value.last_event.data)||(value.session!==null&&!isObject(value.session))||(value.messages!==undefined&&!Array.isArray(value.messages))||(value.evidence!==undefined&&!Array.isArray(value.evidence))) throw new ResearchClientError("invalid interrupt checkpoint response",200,value); if(value.session!==null&&(typeof value.session.session_id!=="string"||!Number.isInteger(value.session.revision)||typeof value.session.storage_ref!=="string")) throw new ResearchClientError("invalid interrupt checkpoint session",200,value); return value as unknown as InterruptCheckpoint; }
