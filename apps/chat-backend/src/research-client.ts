...
function parseRun(value: unknown): ResearchRun { const statuses = ["running", "succeeded", "failed", "cancelled"]; if (!isObject(value) || typeof value.id !== "string" || typeof value.chat_id !== "string" || typeof value.pi_session_id !== "string" || typeof value.model !== "string" || !statuses.includes(String(value.status)) || (value.error !== null && value.error !== undefined && !isObject(value.error))) throw new ResearchClientError("invalid run response", 200, value); return value as unknown as ResearchRun; }
