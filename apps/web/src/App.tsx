...
export function App({ api = createChatApi(), runtime }: { api?: ChatApi; runtime?: WorkbenchRuntime }) {
  ...
  const handleEvent = (event: ChatEvent) => { const type = event.type.toUpperCase(); if (type === "RUN_STARTED" || type === "TOOL_CALL_START") { setActive(true); setTrace((items) => [...items, String(event.data.tool_name ?? "研究中")]); } if (type === "TEXT_MESSAGE_CONTENT") setMessages((items) => { const id = String(event.data.message_id ?? `stream-${event.data.run_id ?? "current"}`); const index = items.findIndex((item) => item.id === id); const content = String(event.data.delta ?? event.data.content ?? ""); if (index >= 0) return items.map((item, i) => i === index ? { ...item, content: item.content + content } : item); return [...items, { id, role: "assistant", content }]; }); if (type === "TEXT_MESSAGE_END" || type === "RUN_FINISHED" || type === "RUN_ERROR") setActive(false); if (type === "RUN_ERROR") setError(String(event.data.message ?? "研究失败")); if (type === "TOOL_CALL_RESULT") { const id = String(event.data.tool_call_id ?? event.id); setInspectorRecords((items) => upsertInspectorRecord(items, { kind: "tool", id, name: String(event.data.tool_name ?? "工具调用"), parameters: event.data.parameters, result: event.data.result })); } };
