...
class AguiStore:
    ...
    def transition_run(self, run_id, status, error=None, emit_event=True, event_type=None, event_data=None):
        ...
        with self.database.transaction() as c:
            ...
            terminal = {"completed", "interrupted", "failed", "cancelled"}
            if row[3] in terminal and row[3] != status:
                raise ValueError(f"terminal run status is immutable: {row[3]}")
            if row[3] != status:
                ...
                if emit_event:
                    seq = self._next_sequence(c, row[1]); typ = event_type or ("RUN_FINISHED" if status in {"completed","interrupted","failed","cancelled"} else "RUN_STARTED")
                    payload = event_data if event_data is not None else {"status": status, "error": error}
                    c.execute("INSERT INTO agui_events(thread_id,sequence,run_id,event_type,payload_json) VALUES (?,?,?,?,?::JSON)", [row[1], seq, run_id, typ, canonical_json(payload)])
                row = c.execute("SELECT id,thread_id,idempotency_key,status,model,input_hash,created_at FROM runs WHERE id=?", [run_id]).fetchone()
