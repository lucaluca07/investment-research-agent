...
IRA_RESEARCH_SERVICE_URL=http://127.0.0.1:8020 PORT=8030 pnpm --dir "$ROOT_DIR" --filter @ira/copilot-runtime dev & PIDS+=("$!")
pnpm --dir "$ROOT_DIR" --filter @ira/web dev --host 127.0.0.1 --port 5173 & PIDS+=("$!")

for endpoint in "http://127.0.0.1:8010/health" "http://127.0.0.1:8020/health" "http://127.0.0.1:8030/health"; do
  ready=0
